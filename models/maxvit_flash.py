...
class RelativePositionalMultiHeadAttention(nn.Module):
    """使用 FlashSelfAttention 优化的相对位置多头注意力"""
    ...
    def __init__(
        ...
        if feat_dim % head_dim != 0:
            raise ValueError(f"feat_dim: {feat_dim}必须能被head_dim: {head_dim}整除")
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # 调整形状以适配 FlashSelfAttention 的输入格式
        q = q.reshape(B * G, P, H, DH).permute(0, 2, 1, 3)  # [B*G, H, P, DH]
        k = k.reshape(B * G, P, H, DH).permute(0, 2, 1, 3)
        v = v.reshape(B * G, P, H, DH).permute(0, 2, 1, 3)

        # 合并 QKV 并应用 FlashSelfAttention
        qkv = torch.stack([q, k, v], dim=2)  # [B*G, H, 3, P, DH]
        attn_output = self.flash_attn(qkv)

        # 重新调整形状
        out = attn_output.permute(0, 2, 3, 1).reshape(B, G, P, D)
        out = self.merge(out)
        return out
...
def maxvit_t_flash(*, weights: Optional[MaxVit_T_Weights] = None, progress: bool = True, **kwargs: Any) -> MaxVit:
    """
