...
def _get_relative_position_index(height: int, width: int) -> torch.Tensor:
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
        super().__init__()
        self.depthwise = Conv2d(in_channels, in_channels, kernel_size, stride, padding, groups=in_channels)
...
class FusedMBConv(nn.Module):
    """Fused-MBConv: Merged Mobile Inverted Bottleneck Block.

    Args:
        in_channels (int): 輸入通道數。
        out_channels (int): 輸出通道數。
        expansion_ratio (float): 擴展比率（用於中間層通道擴增）。
        squeeze_ratio (float): SE模塊的通道壓縮比率。
        stride (int): 卷積步長。
        activation_layer (Callable[..., nn.Module]): 激活函數。
        norm_layer (Callable[..., nn.Module]): 歸一化層。
        p_stochastic_dropout (float): 隨機深度概率。
    """
    ...
    def __init__(
        ...
        # --- 核心修改：合併擴展卷積與深度卷積為單個常規卷積 ---
        _layers = OrderedDict()
        _layers["pre_norm"] = norm_layer(in_channels)
        
        # 合併後的 3x3 常規卷積（關鍵修改點：groups=1）
        _layers["conv_fused"] = Conv2dNormActivation(
            in_channels,
            mid_channels,
            kernel_size=3,
            stride=stride,
            ...
            activation_layer=activation_layer,
            norm_layer=norm_layer,
            groups=1,  # 不再是深度卷積（groups=mid_channels）
            inplace=None,
        )
        ...
        # 降維卷積（保持不變）
        _layers["conv_c"] = nn.Conv2d(mid_channels, out_channels, kernel_size=1, bias=True)
...
class MaxVitLayer(nn.Module):
    ...
    def __init__(
        ...
        if use_fused:
            # Fused MBconv
            layers["fused_MBconv"] = FusedMBConv(
...
class MaxVit(nn.Module):
    ...
    def __init__(
        ...
        p_idx = 0
        for block_idx,(in_channel, out_channel, num_layers) in enumerate(zip(in_channels, out_channels, block_layers)):
            self.blocks.append(
                MaxVitBlock(
                    use_fused=(block_idx < 2),
...
def maxvit_t_fused(*, weights: Optional[MaxVit_T_Weights] = None, progress: bool = True, **kwargs: Any) -> MaxVit:
    """
